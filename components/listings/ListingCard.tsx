    ...
    <Card className="overflow-hidden rounded-2xl shadow-sm">
      <div className="relative h-40 w-full bg-gray-100">
        ...
        <button
          ...
          className={`absolute top-2 right-2 rounded-full p-2 bg-white/80 backdrop-blur ${
            saved.includes(l.id) ? "text-red-600" : "text-gray-600"
          }`}
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-3 text-gray-600">
          ...
          {l.endDate && (
            <LabelRow icon={CalendarDays} label={`End ${new Date(l.endDate).toLocaleDateString()}`} />
          )}
        ...
        <div className="pt-2 flex items-center justify-between">
          <button
            ...
            onClick={() => alert(`Contact: ${l.contact}`)}
          >
            Contact
          </button>
          <button
            className="rounded-md px-3 py-1 text-sm bg-gray-900 text-white"
            onClick={() => alert("Report submitted. (Moderation queue soon)")}
          >
            Report
          </button>
        </div>
